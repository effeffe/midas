...
class EquipmentBase:
    ...
    def _get_event(self):
        ...
        if event.header.serial_number is None:
            event.header.serial_number = self.stats["events"]
...
class FrontendBase:
            ...
            for equip in self._enabled_equipment():
                if equip._is_active_for_state(self.run_state):
                    if equip._is_polled() and equip.poll_func():
                        ...
                        self._get_and_send_event(equip)
    ...
    def _get_and_send_event(self, equip):
        """
        Call _get_event() (which call's the user's readout_func()) for the
        given equipment.
